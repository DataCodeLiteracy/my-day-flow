...
export default function WeeklyAnalyticsPage() {
  ...
  }, [isLoggedIn, user?.uid, selectedWeek])
  ...
  const loadWeeklyAnalytics = async () => {
    try {
      ...
      const weekSessions = sessions.filter((session) => {
        ...
        return sessionDate >= selectedWeek && sessionDate <= weekEnd
      })

      // 분석 데이터 계산
      const analysisData = calculateWeeklyAnalytics(
        weekSessions,
        categoriesData,
        flatItems,
        selectedWeek,
      ...
      console.error("Error loading weekly analytics data:", error)
    } finally {
      setIsLoading(false)
    }
  }
            ...
            <div className='bg-theme-secondary rounded-2xl p-6 mb-6 shadow-lg'>
              ...
              <div className='space-y-4'>
                ...
                <div className='flex items-center justify-between p-3 bg-theme-primary/5 rounded-lg'>
                  ...
                  <div className='text-xl font-bold text-accent-theme'>
                    {formatTime(Math.floor(analyticsData.totalTime / 7))}
                  </div>
                </div>
                <div className='flex items-center justify-between p-3 bg-theme-primary/5 rounded-lg'>
                    ...
                    .map(([hour, time]) => (
                      ...
                      >
                        ...
                        <div className='text-right'>
                          ...
                          <div className='text-xs text-theme-tertiary'>
                            하루 평균 {formatDuration(Math.floor(time / 7))}
                          </div>
                        </div>
                      </div>
                    ))}
